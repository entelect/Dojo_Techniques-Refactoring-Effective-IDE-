...
public class CalculatorTest {
    ...
    public void testAddBankAccount(){
        ...
        calculator.addBankAccount(savingsAccount);
    ...
    public void testUpdateBalance(){
        ...
        for (int i = 0; i < bankAccounts.length; i++) {
            calculator.calculateBalance(bankAccounts[i], -500);
            assertEquals(500, bankAccounts[i].getBalanceInCents());

            calculator.calculateBalance(bankAccounts[i], 1000);
    ...
    public void testUpdateSavingsAccountBalanceFail(){
        createTestAccounts(INITIAL_BALANCE);
        calculator.calculateBalance(savingsAccount, -INITIAL_BALANCE * 2);
    }

    @Test(expected = BankAccountException.class)
    public void testUpdateMoneyMarketAccountBalanceFail(){
        createTestAccounts(INITIAL_BALANCE);
        calculator.calculateBalance(moneyMarketAccount, -INITIAL_BALANCE * 2);
    }
